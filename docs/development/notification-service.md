...
- 管理接口使用独立无状态资源服务器链，只接受 RS256，并校验 issuer、`notification-service` audience、调用方白名单、`notification.smtp.manage` scope、签发与过期时间及最大有效跨度。Notification 只验证 Token，不持有或签发私钥。
...
每个 `SMTP_TEST` 任务同时记录被测试的配置版本，`smtp_config` 只保留最新测试任务及其目标版本。Worker 回写结果时必须同时匹配最新任务标识；配置更新会清空该关联，旧任务不得覆盖新配置或新测试的状态。
...
- `NOTIFICATION_SECRET_STORE_MASTER_KEY_FILE`
- `NOTIFICATION_SECRET_STORE_KEY_ID`（默认 `local-v1`）
- `NOTIFICATION_SERVICE_JWT_ISSUER`
- `NOTIFICATION_SERVICE_JWT_JWK_SET_URI`（必须为 HTTPS）
- `NOTIFICATION_SERVICE_JWT_AUDIENCE`（默认 `notification-service`）
- `NOTIFICATION_SERVICE_JWT_ALLOWED_CALLERS`（默认 `gateway-service`）
- `NOTIFICATION_SERVICE_JWT_MAXIMUM_LIFETIME`（默认 `PT5M`，且不超过 1 小时）

SMTP 管理权限固定要求 `notification.smtp.manage` scope，不允许通过环境变量改写。
