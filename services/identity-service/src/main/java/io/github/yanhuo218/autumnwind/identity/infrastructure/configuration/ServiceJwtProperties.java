...
@ConfigurationProperties("autumn-wind.identity.service-jwt")
public record ServiceJwtProperties(
    ...
    public ServiceJwtProperties {
        ...
        requiredScope = requireText(requiredScope, "Service JWT scope 不能为空。");
        if (!requiredScope.matches("[A-Za-z0-9._:-]{1,128}")) {
            throw new IllegalArgumentException("Service JWT scope 格式不合法。");
        }
        ...
        if (!jwkSetUri.isAbsolute()
                || !"https".equalsIgnoreCase(jwkSetUri.getScheme())
                || jwkSetUri.getHost() == null
                || jwkSetUri.getUserInfo() != null
