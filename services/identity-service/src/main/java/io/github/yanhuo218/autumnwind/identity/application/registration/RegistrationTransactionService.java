...
public class RegistrationTransactionService {
    ...
    public void create(RegisterCommand command) {
        Objects.requireNonNull(command, "注册命令不能为空。");
        AuthPolicyEntity policy = authPolicyRepository.findById(POLICY_ID)
                .orElseThrow(RegistrationTransactionService::policyUnavailable);
        RegistrationPolicy registrationPolicy = policy.registrationPolicy();
        if (!registrationPolicy.publicRegistrationEnabled()) {
            throw registrationNotAllowed();
        }

        NormalizedEmail email = normalizeEmail(command.email());
        if (!registrationPolicy.allowsPublicRegistration(email)) {
            throw registrationNotAllowed();
        }
        if (registrationPolicy.emailVerificationRequired() || policy.requiresAcceptanceAudit()) {
            throw new IdentityApplicationException(
                    IdentityErrorCode.REGISTRATION_UNAVAILABLE,
                    "当前注册策略依赖尚未启用的身份验证能力。"
            );
        }
    ...
    private static IdentityApplicationException registrationNotAllowed() {
        return new IdentityApplicationException(
