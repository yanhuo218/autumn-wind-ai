...
import io.github.yanhuo218.autumnwind.gateway.model.ModelRegistryProxyClient;
...
public class GatewayWebClientConfiguration {
    ...
    @Bean("modelRegistryWebClient")
    public WebClient modelRegistryWebClient(GatewayDownstreamProperties properties) {
        ExchangeStrategies strategies = ExchangeStrategies.builder()
                .codecs(codecs -> codecs.defaultCodecs().maxInMemorySize(MAX_RESPONSE_BODY_BYTES))
                .build();
        HttpClient httpClient = HttpClient.create().responseTimeout(TOTAL_TIMEOUT);
        return WebClient.builder()
                .baseUrl(properties.modelRegistryBaseUrl().toString())
                .clientConnector(new ReactorClientHttpConnector(httpClient))
                .exchangeStrategies(strategies)
                .build();
    }
    ...
    @Bean
    public ModelRegistryProxyClient modelRegistryProxyClient(
            @Qualifier("modelRegistryWebClient") WebClient modelRegistryWebClient,
            ServiceJwtIssuer serviceJwtIssuer,
            ObjectMapper objectMapper
    ) {
        return ModelRegistryProxyClient.webClientBacked(modelRegistryWebClient, serviceJwtIssuer, objectMapper);
    }
}
